...
drchrono_auth = DrChronoClient( # drchrono_auth is used to pass in client_id, client_secret, redirect_uri.
    ...
    # client_id and client_secret are given to the developer by drchrono through the drchrono website
    client_id = os.getenv('CLIENT_ID'),
    client_secret = os.getenv('CLIENT_SECRET'),
    # redirect_uri is chosen by the developer and configured in the drchrono website
    redirect_uri = "http://localhost:5000/drchrono/oauth2callback",
)

app = Flask(__name__) # initialize the Flask framework

app.config['SECRET_KEY'] = os.urandom(32) # set the secret key

@app.route("/") # redirects the user to /drchrono/
def index():
...
def drchrono_oauth2callback():
    # check for and print a friendly error message
    error = request.args.get("error")
    if error:
        return "error :( {!r}".format(error) # prints error if there is one TODO: test if I get an error statement. Test scope, response type
